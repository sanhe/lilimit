...
The `release-linux` GitHub Actions workflow builds these artifacts on Ubuntu 22.04, uploads them to the workflow run as `lilimit-ubuntu-packages`, and attaches them to a draft GitHub release. Push a tag like `lilimit-v0.1.4`, then run the `release-macos` workflow on the same tag. Publish the draft after checking both platforms.
